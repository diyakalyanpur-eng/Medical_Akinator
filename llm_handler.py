...
#         """Use LLM to answer student's medical question"""
#         prompt = f"""You are a medical educator. A student is trying to diagnose a disease by asking questions.
...
# Rules:
# 1. Answer with YES, NO, or SOMETIMES
# 2. If a symptom/feature is NOT mentioned in the disease data, answer NO and briefly explain why it's typically absent for this condition
# 3. Be medically accurate based on the data
# 4. Keep your response brief and educational

# Respond in this exact JSON format (no extra text):
# {{"answer": "YES/NO/SOMETIMES", "explanation": "brief medical reason"}}"""
...
#             }
    def answer_medical_question(self, disease_data, disease_name, question):
        ...
        # ==================== LLM-BASED HANDLING ====================
        # For complex clinical reasoning questions, use the LLM
    ...
    Respond in this exact JSON format (no extra text):
    {"answer": "specific answer based on data", "explanation": "brief medical context"}"""
    ...
    Task: Based ONLY on the disease information provided, answer the student's question accurately.
...
#     def update_differential_diagnosis(self, disease_name, questions_asked, all_diseases):
#         """Generate differential diagnosis based on questions asked so far"""
#         findings = []
#         for qa in questions_asked:
#             if qa['answer'] == 'YES':
#                 findings.append(f"{qa['question']}: YES")
#             elif qa['answer'] == 'NO':
#                 findings.append(f"{qa['question']}: NO")
...
# Available diseases to consider:
# {json.dumps([d['name'] for d in all_diseases[:10]], indent=2)}

# Task: Based on the findings, list the TOP 3 most likely diseases in order of probability.

# Respond in this exact JSON format:
# {{
#   "differentials": [
#     {{"disease": "Disease Name", "likelihood": "most likely", "reasoning": "brief reason"}},
#     {{"disease": "Disease Name", "likelihood": "possible", "reasoning": "brief reason"}},
#     {{"disease": "Disease Name", "likelihood": "less likely", "reasoning": "brief reason"}}
#   ]
# }}"""
...
#             return []
    def update_differential_diagnosis(self, disease_name, questions_asked, all_diseases, disease_data=None):
        ...
        findings = []
        for qa in questions_asked:
            if qa['answer'] in ['YES', 'NO']:  # Only include definitive answers
...
{json.dumps(essential_features, indent=2)}
  ...
  "feedback": "1-2 sentence constructive feedback"
        ...
        if not response:
            return {
                ...
                "essential_asked": 0,
                "essential_missed": len(essential_features),
